...
class FakingRecipe_Dataset(Dataset):
    def __init__(self, vid_path,dataset):
        ...
        base_path = '/home/user/D/32T/lpj/video/data/fakett/'
        if dataset=='fakesv':
            self.data_all = pd.read_json(base_path+'fea/fakesv/metainfo.json',orient='records',dtype=False,lines=True)
            self.vid=[]
            ...
            self.ocr_pattern_fea_path=base_path+'fea/fakesv/preprocess_ocr/sam'
            self.ocr_phrase_fea_path=base_path+'fea/fakesv/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path=base_path+'fea/fakesv/preprocess_text/768_sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path=base_path+'fea/fakesv/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.text_emo_fea = torch.load(f)

            self.audio_fea_path=base_path+'fea/fakesv/preprocess_audio'
            self.visual_fea_path=base_path+'fea/fakesv/preprocess_visual'
        elif dataset=='fakett':
            self.data_all=pd.read_json(base_path+'fea/fakett/metainfo.json',orient='records',lines=True,dtype={'video_id': str})
            self.vid=[]
            with open(vid_path, "r") as fr:
                for line in fr.readlines():
                    self.vid.append(line.strip())
            self.data = self.data_all[self.data_all.video_id.isin(self.vid)]
            self.data.reset_index(inplace=True)
            self.ocr_pattern_fea_path=base_path+'fea/fakett/preprocess_ocr/sam'

            self.ocr_phrase_fea_path=base_path+'fea/fakett/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path=base_path+'fea/fakett/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path=base_path+'fea/fakett/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.text_emo_fea = torch.load(f)

            self.audio_fea_path=base_path+'fea/fakett/preprocess_audio'
            self.visual_fea_path=base_path+'fea/fakett/preprocess_visual'
    ...
    def __getitem__(self, idx):
        ...
        raw_visual_frames=torch.tensor(torch.load(open(v_fea_path,'rb'), weights_only=False))
        ...
        ocr_pattern_fea=torch.tensor(torch.load(open(ocr_pattern_fea_file_path,'rb'), weights_only=False))
...
def collate_fn_FakeingRecipe(batch):
    ...
    # raw_visual_frames 是所有的frame切片，以及对应的特征，比如(444, 512)就是有440个512特征
    # num_visual_frames 是最大的frame数量
    # visual_time_region 其中的数字是切片开始，结束时间
    # sampled_seg，补齐了切片的片段
    # visual_frames_seg_indicator 不同切片的示意，采用0，1，2，3，4，5等示意
    # visual_frames_fea 切片特征
