...
class Trainer():
    def __init__(self,model,device,lr,dataloaders,save_param_path,writer,early_stop,epoches,model_name,save_predict_result_path,model_times,beta_c,beta_n,scheduler_option=False,save_threshold = 0.7, start_epoch = 0):
        self.model = model
        self.device = device
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.start_epoch+self.num_epochs):
            ...
            for batch in tqdm(self.dataloaders['train']):
                ...
                # outputs, output_meam = self.model(**batch_data)
            ...
            if results_val['f1']>best_f1_test:
                ...
                if best_f1_test > self.save_threshold:
                    torch.save(best_model_wts_test, self.save_param_path+self.model_name+"_test_" + str(best_epoch_test) + "_{0:.4f}".format(best_f1_test))
                    print("saved "+self.save_param_path+self.model_name+ "_test_" + str(best_epoch_test) + "_{0:.4f}".format(best_f1_test))
            else:
        ...
        print("Best model on test: epoch" + str(best_epoch_test) + "_" + str(best_f1_test))
        ckp_path=self.save_param_path+self.model_name+"_test_" + str(best_epoch_test) + "_{0:.4f}".format(best_f1_test)
    ...
    def test(self,ckp_path):
        ...
        result.to_csv(self.save_predict_result_path+self.model_name+self.model_times+'.csv',sep='\t',index=False)
