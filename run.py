...
class Run():
    def __init__(self,config):
        ...
        self.model_times = config['model_times']
    ...
    def main(self):
        self.model = D_query_frame(self.dataset)
        ...
        if self.mode=='train':
            ...
            trainer=Trainer(model=self.model,device=self.device,lr=self.lr,dataloaders=dataloaders,epoches=self.epoches,model_name='FakingRecipe',save_predict_result_path=save_predict_result_path,model_times=self.model_times,beta_c=self.alpha,beta_n=self.beta,early_stop=self.early_stop,save_param_path=self.path_ckp+self.dataset+"/"+self.model_times+"/",writer=SummaryWriter(self.path_tb+self.dataset+"/"+self.dataset+'_'+self.model_times+"/"))
            ...
            inferncer=Inferencer(model=self.model,device=self.device,model_name='FakingRecipe'+self.model_times,dataset=self.dataset,dataloader=dataloader,save_predict_result_path=save_predict_result_path)
